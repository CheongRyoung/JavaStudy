	...
	public static void main(String[] args) {
		...
		try {
			...
			while(true) {
				...
				System.out.println("누군가 접속함, 접속인원: " + memberManager.getMemberCount());
				memberThread.start();
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			...
			while(true) {
				...
				if(command==1) {
					...
					String message = "[공지] "+ nickName + "님이 입장하였습니다.";
					...
					String message = nickName + "님이 퇴장 하셨습니다.";
					memberManager.broadCast(message);
